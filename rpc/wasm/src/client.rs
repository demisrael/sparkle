...
declare! {
     ...
     */
    export interface ISparkleRpcConfig {
        /**
...
impl Default for SparkleRpcConfig {
    fn default() -> Self {
        RpcConfig { url: None, encoding: Some(Encoding::Borsh), network_id: None, resolver: None }
    }
}

impl TryFrom<ISparkleRpcConfig> for SparkleRpcConfig {
    type Error = Error;
    fn try_from(config: ISparkleRpcConfig) -> Result<Self> {
        ...
        Ok(SparkleRpcConfig { resolver, url, encoding, network_id })
    }
}

impl TryFrom<SparkleRpcConfig> for ISparkleRpcConfig {
    type Error = Error;
    fn try_from(config: SparkleRpcConfig) -> Result<Self> {
        let object = ISparkleRpcConfig::default();
...
impl RpcClient {
    pub fn new(config: Option<SparkleRpcConfig>) -> Result<SparkleRpcClient> {
        let SparkleRpcConfig { resolver, url, encoding, network_id } = config.unwrap_or_default();
...
impl RpcClient {
    ...
    ///
    /// @see {@link ISparkleRpcConfig} interface for more details.
    ///
    #[wasm_bindgen(constructor)]
    pub fn ctor(config: Option<ISparkleRpcConfig>) -> Result<SparkleRpcClient> {
        Self::new(config.map(SparkleRpcConfig::try_from).transpose()?)
    }
