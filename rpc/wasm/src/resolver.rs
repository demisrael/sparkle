...
use kaspa_wrpc_client::Resolver as NativeSparkleResolver;
use serde::ser;
use workflow_wasm::extensions::ObjectExtension;

declare! {
    IResolverConfig,
    "ISparkleResolverConfig | string[]",
    r#"
    /**
     ...
     */
    export interface ISparkleResolverConfig {
        /**
...
declare! {
    ISparkleResolverConnect,
    "ISparkleResolverConnect | NetworkId | string",
    r#"
    /**
     ...
     * @category Sparkle RPC
     */
    export interface ISparkleResolverConnect {
        /**
...
extern "C" {
    ...
    pub type SparkleResolverArrayT;
}


#[derive(Debug, Clone, CastFromJs)]
#[wasm_bindgen(inspectable)]
pub struct SparkleResolver {
    resolver: NativeSparkleResolver,
}

impl SparkleResolver {
    pub fn new(resolver: NativeSparkleResolver) -> Self {
        Self { resolver }
    }
}
...
impl SparkleResolver {
    ...
    pub fn ctor(args: Option<ISparkleResolverConfig>) -> Result<SparkleResolver> {
        if let Some(args) = args {
            Ok(Self { resolver: NativeSparkleResolver::try_from(args)? })
        } else {
            Ok(Self { resolver: NativeSparkleResolver::default() })
        }
    }
}
...
impl Resolver {
    ...
    #[wasm_bindgen(getter)]
    pub fn urls(&self) -> SparkleResolverArrayT {
    ...
    pub async fn connect(&self, options: ISparkleResolverConnect) -> Result<RpcClient> {
...
impl TryFrom<ISparkleResolverConfig> for NativeSparkleResolver {
    type Error = Error;
    fn try_from(config: IResolverConfig) -> Result<Self> {
        let resolver = config
            ...
            .map(|urls| NativeSparkleResolver::new(urls.into_iter().map(Arc::new).collect()));

        Ok(resolver.unwrap_or_default())
    }
}

impl TryCastFromJs for SparkleResolver {
    type Error = Error;
...
impl TryFrom<&JsValue> for SparkleResolver {
    type Error = Error;
...
impl From<SparkleResolver> for NativeSparkleResolver {
    fn from(resolver: Resolver) -> Self {
        resolver.resolver
    }
}

impl From<NativeSparkleResolver> for SparkleResolver {
    fn from(resolver: NativeSparkleResolver) -> Self {
        Self { resolver }
    }
}
