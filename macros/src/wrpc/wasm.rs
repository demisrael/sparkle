    ...
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if parsed.len() != 2 {
            return Err(Error::new_spanned(
                parsed,
                "usage: build_wrpc_wasm_bindgen_interface!([fn no args, ..],[fn with args, ..])"
                    .to_string(),
            ));
        }
...
pub fn build_wrpc_wasm_bindgen_interface(
...
impl ToTokens for RpcSubscriptions {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        for handler in self.handlers.elems.iter() {
            ...
            let fn_unsubscribe_snake =
                Ident::new(&unsubscribe.to_case(Case::Snake), Span::call_site());
            let fn_unsubscribe_camel =
                Ident::new(&unsubscribe.to_case(Case::Camel), Span::call_site());
...
pub fn build_wrpc_wasm_bindgen_subscriptions(
    input: proc_macro::TokenStream,
) -> proc_macro::TokenStream {
        ...
        if parsed.len() == 2 {
            ...
            let alias = match iter.next().unwrap().clone() {
                Expr::Lit(ExprLit {
                    lit: Lit::Str(lit_str),
                    ..
                }) => Literal::string(&lit_str.value()),
                _ => {
                    return Err(Error::new_spanned(
                        parsed,
                        "type spec must be a string literal".to_string(),
                    ))
                }
            };
            let declaration = extract_literal(&iter.next().unwrap().clone())?;
            ...
            return Err(Error::new_spanned(
                parsed,
                "usage: declare_wasm_interface!(typescript_type, [alias], typescript declaration)"
                    .to_string(),
            ));
        }
    }
...
pub fn declare_typescript_wasm_interface(
    input: proc_macro::TokenStream,
) -> proc_macro::TokenStream {
...
fn insert_typedoc(text: &str, insertion: &str) -> String {
    if let Some(mut index) = text.find("/**") {
        ...
        let insertion = insertion
            .split('\n')
            .filter_map(|line| {
                (!line.trim().is_empty()).then_some(format!("\n\t* {}", line.trim()))
            })
            .collect::<String>();
