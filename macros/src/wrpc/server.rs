...
impl ToTokens for RpcTable {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        for handler in self.handlers.elems.iter() {
            ...
            targets.push(quote! {
                #rpc_api_ops::#handler => {
                    interface.method(#rpc_api_ops::#handler, method!(|server_ctx: #server_ctx_type, connection_ctx: #connection_ctx_type, request: workflow_serializer::prelude::Serializable<#request_type>| async move {
                        let verbose = server_ctx.verbose();
                        if verbose { workflow_log::log_info!("request: {:?}",request); }
                        let response: #response_type = server_ctx.nexus().#fn_call(&connection_ctx, request.into_inner()).await
                            .map_err(|e|ServerError::Text(e.to_string()))?;
                        if verbose { workflow_log::log_info!("response: {:?}",response); }
                        Ok(workflow_serializer::prelude::Serializable(response))
                    }));
                }
            });
        }
